 ...
 */
 ...
 loadAPI(1);
 host.defineController("Native Instruments Community", "communityScript", "420.0", "1543a786-0e21-49a2-bbed-ec603d7795cf");
 ...
 load("Constants.js");
 load("JamMode.js");
 load("BrowseModeTable.js");

 load("MultiMidiMode.js");
 load("MultiMidiView.js");
 load("BrowseMode.js");
 load("ClipMode.js");
 load("PadMode.js");
 load("StepMode.js");
...
function init() {
	...
	transport = new TransportHandler(trackHandler, cursorDevice,extension);
	...
	var browseView 	= new BrowseView(noteInput);
	var multiMidiView = new MultiMidiView(noteInput);
	...
	var modeHandler = new ModeHandler(cliplaunchMode, padMode, stepMode, effectMode, browseMode, clip, trackHandler, trackViewContainer,multiMidiMode);

	var mainKnobControl = new MainKnobKontrol(cursorTrack, transport.transport(), clip, cursorDevice, modeHandler);
	...
	if(previousMode === null || previousMode === undefined){
		println("previousMode is undefined");
	}
	println(previousMode + "previousMode");
 ...
 * 
 */
 function ModeHandler(cliplaunchMode, padMode, stepMode, effectMode, browseMode, cursorClip, trackHandler, trackViewContainer,multiMidiMode) {
 	...
 	browseMode.selbutton = new EmptyButton();
 	multiMidiMode.selbutton = new EmptyButton();
 	...
 	this.enterBrowseMode = function (){
		...
		currentMode.exit();
		browseMode.enter();
		browseMode.setModeHandler(this);
		currentMode = browseMode;
	}

	this.exitBrowseMode = function (){
		currentMode.exit();
		if(undefined !== previousMode && null !== previousMode){
			previousMode.enter();
		}else{
			cliplaunchMode.enter();
		}
	...
	this.exitBrowseAndEnterPadMode = function(){
		currentMode.exit();
		...
		if (value > 0) {
			 if (currentMode === stepMode) {
				if (modifiers.isSelectDown()) {
					//trackHandler.selectNextSlotInTrack(); OLD!!
					currentMode.exit();
					multiMidiMode.enter();
					multiMidiMode.setModeHandler(this);
					currentMode = multiMidiMode;
...
function initLeftButtons() {
	...
	dirPadUp.setCallback(
		function (value) {
			if (value !== 0) {
				if (knobControl.getBrowsing() && modifiers.isShiftDown()) {
