 ...
 * @param {ModeHandler} modeHandler eig nur da um browsezustand an den mode handler zu übergeben und den browse mode zu starten - hacks ftw!
 */
function MainKnobKontrol(cursorTrack, transport, cursorClip, cursorDevice, modeHandler) {
    var Modes = {
        MST: 0,
        GROUP: 1,
        METRO: 2,
    ...
    var userWantsBrowseMode = true;
    var cursorDeviceName = ""

    cursorDevice.addNameObserver(99,"",function(name){
        cursorDeviceName = name;
        host.showPopupNotification("name: " + name);
    ...
    browserButton.setCallback(function (value) {
        ...
        if (browsing) {
            ...
            host.showPopupNotification("cancel browse");
            modeHandler.exitBrowseMode();
            //if cursordevice = none
            //-> devices
            //modehandler.enterBrowseMode

            //if cursordevice = sample -> choose sample mode
            ...
            host.showPopupNotification("start the browse - name: " + cursorDeviceName);
